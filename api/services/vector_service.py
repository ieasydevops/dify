...
class VectorService:
    ...
    def update_child_chunk_vector(
        ...
        # 处理需要删除的块
        delete_node_ids.extend(chunk.index_node_id for chunk in delete_child_chunks)
        
        # 处理需要更新的块
        delete_node_ids.extend(chunk.index_node_id for chunk in update_child_chunks)
        documents.extend(
            Document(
                page_content=chunk.content,
                metadata={
                    "doc_id": chunk.index_node_id,
                    "doc_hash": chunk.index_node_hash,
                    "document_id": chunk.document_id,
                    "dataset_id": chunk.dataset_id,
                },
            )
            for chunk in update_child_chunks
        )
